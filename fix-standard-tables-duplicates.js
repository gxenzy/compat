...
async function fixStandardTablesDuplicates() {
  let connection;
  try {
    ...
    // First, let's back up the standard_tables table
    console.log('Creating backup of standard_tables table...');
    await connection.query(`CREATE TABLE IF NOT EXISTS standard_tables_backup LIKE standard_tables`);
    await connection.query(`TRUNCATE TABLE standard_tables_backup`);
    await connection.query(`INSERT INTO standard_tables_backup SELECT * FROM standard_tables`);
    console.log('Backup created as standard_tables_backup');
    
    // Check for duplicates in standard_tables table
    ...
    if (duplicates.length === 0) {
      console.log('No duplicates found in standard_tables table');
      return;
    }
    
    console.log(`Found ${duplicates.length} sets of duplicates in standard_tables table.`);
    ...
    try {
      ...
      for (const duplicate of duplicates) {
        const { section_id, table_number } = duplicate;
        ...
        // First, identify tables with foreign keys to standard_tables
        const [fkTables] = await connection.query(`
          ...
          WHERE 
            REFERENCED_TABLE_NAME = 'standard_tables' 
        ...
        console.log(`Deleted ${deleteResult.affectedRows} duplicate records for section_id ${section_id}, table ${table_number}`);
      }
      ...
      try {
        await connection.query(`
          ...
          ADD CONSTRAINT unique_standard_table UNIQUE (section_id, table_number)
        `);
        console.log('Added unique constraint to standard_tables table');
    ...
    const [afterFix] = await connection.query(`
      SELECT section_id, table_number, COUNT(*) as count 
      FROM standard_tables 
      GROUP BY section_id, table_number 
      HAVING COUNT(*) > 1
    `);
    ...
    if (afterFix.length === 0) {
      console.log('Verification successful: No more duplicates in standard_tables table');
    } else {
...
module.exports = { fixStandardTablesDuplicates };

// Uncomment to run this script standalone
// fixStandardTablesDuplicates(); 