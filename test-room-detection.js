/**
 * Test Room Detection Script
 * 
 * This script processes a floor plan image through the OpenCV-based room detection pipeline
 * to test the detection capabilities without requiring the full web application.
 * 
 * Usage:
 *   node test-room-detection.js <path_to_floor_plan>
 */
...
const SAMPLE_IMAGE_PATH = path.join(WORKSPACE_ROOT, 'reference book', 'sample-floorplan.jpg');
...
// Create a sample floor plan image if it doesn't exist
...
print(f"Sample image saved to ${SAMPLE_IMAGE_PATH}")
  ...
  console.log(`Using existing sample image: ${SAMPLE_IMAGE_PATH}`);
  runDetection(SAMPLE_IMAGE_PATH);
}
